        ...
        <div className="space-y-6">
          <div className="p-6 border border-default rounded-2xl bg-subtle space-y-4">
            ...
            <div className="grid grid-cols-4 gap-4 items-center p-4 border border-default rounded-xl bg-surface text-sm">
              <div className="w-12 h-12 rounded-lg bg-surface border border-default flex items-center justify-center">
      ...
      {subTab === "test" && (
        <div className="p-6 border border-default rounded-2xl bg-subtle space-y-4">
          ...
          <button
            onClick={handleTest}
            disabled={isTesting}
            className="px-6 py-2.5 bg-surface hover-bg-subtle border border-default text-default rounded-xl text-xs font-bold transition-all flex items-center gap-2"
          >
          ...
          {testResult && (
            <div className={`p-4 border rounded-xl text-sm space-y-2 ${testResult.success ? "border-[var(--color-success)]/25 bg-[var(--color-success)]/5" : "border-[var(--color-danger)]/25 bg-[var(--color-danger)]/5"}`}>
              <div className="flex items-center gap-2 font-bold text-default">
                {testResult.success ? <CheckCircle2 className="w-4 h-4 text-[var(--color-success)]" /> : <XCircle className="w-4 h-4 text-[var(--color-danger)]" />}
              ...
              {testResult.error && <p className="text-xs text-[var(--color-danger)]">{testResult.error}</p>}
              ...
              {usageLogs.map((log) => (
                <div key={log.id} className="p-3 bg-surface border border-default rounded-xl text-[10px] font-mono">
                  <div className="flex justify-between text-muted mb-1">
                    ...
                    <span className={`px-1.5 py-0.5 rounded font-bold ${log.status === "success" ? "text-[var(--color-success)]" : "text-[var(--color-danger)]"}`}>
                      {log.status}
                    </span>
                  </div>
                  ...
                  {log.error_message && <div className="text-[var(--color-danger)] mt-1 truncate">{log.error_message}</div>}
                </div>
              ))}
            </div>
          )}
