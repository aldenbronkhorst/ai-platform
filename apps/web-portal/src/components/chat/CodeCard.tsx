...
function CodeCardTitle({ className, children, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "flex min-w-0 items-center gap-1.5 truncate text-[length:var(--conversation-tool-font-size)] font-medium leading-[var(--conversation-line-height)] text-foreground/80",
        className,
      )}
...
function CodeCardSubtitle({ className, ...props }: React.ComponentProps<"span">) {
  return <span className={cn("font-normal text-muted-foreground", className)} data-slot="code-card-subtitle" {...props} />;
}
