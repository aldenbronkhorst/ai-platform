...
CANONICAL_TOOL_DEFINITIONS: list[dict[str, Any]] = [
    {
        ...
        "description": "Consolidated Odoo command center. Use one broad mode instead of feature-specific tools. Modes: health, schema, query, count, aggregate, report, attachment, content, message, mutation, execute. Normal ORM work uses XML-RPC; complex accounting reports use the connector report path. Report mode exposes Odoo's own auditable cell drilldowns for amounts that can be clicked in Odoo. Record-level results include verified record_url values when a link can be built; never infer Odoo hostnames. For side effects such as message_post or mail.activity completion, only report success when effect_verified is true; message_post is record chatter, not a private Discuss direct message.",
        "target_system": "odoo",
        "input_schema": {
            "type": "object",
            "properties": {
                "mode": {"type": "string", "enum": ["health", "schema", "query", "count", "aggregate", "report", "attachment", "content", "message", "mutation", "execute"], "description": "Broad operation mode"},
                ...
                "limit": {"type": "integer", "description": "Maximum records to return in one read-only result (default 1000, max 5000)", "default": 1000},
                "offset": {"type": "integer", "description": "Records to skip before the result. Use only when intentionally continuing after the returned limit.", "default": 0},
                ...
                "include_raw_lines": {"type": "boolean", "description": "Include raw Odoo report lines for report debugging", "default": False},
                "include_drilldowns": {"type": "boolean", "description": "For report mode, run Odoo's own audit/drilldown action for auditable report amounts. Use with line_names when the user asks what a report amount is made of.", "default": False},
                "drilldown_limit": {"type": "integer", "description": "Maximum drilldown records to return per auditable report cell (default 1000, max 5000)", "default": 1000},
                "drilldown_offset": {"type": "integer", "description": "Drilldown records to skip before reading results", "default": 0},
                "drilldown_fields": {"type": "array", "items": {"type": "string"}, "description": "Optional fields to return from the model opened by the report drilldown action"},
                ...
                "groupby": {"type": "array", "items": {"type": "string"}, "description": "Fields to group by for aggregate mode"},
            },
            "required": ["mode"],
        },
    },
